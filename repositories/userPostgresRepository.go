...
func (u *userPostgresRepository) GetNormalUser(in *entities.NormalUsers) (*entities.NormalUsers, error) {
	...
	if err := u.db.Where(&in).Preload(clause.Associations).Preload("Teams.Teams").Preload("Teams.Teams.TeamsMembers").Preload("Compatitions.Compatitions").Preload("Compatitions.Compatitions.Matchs").First(normalUser).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("record not found")
		}
		return nil, err
